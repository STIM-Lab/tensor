...
void GenerateGlyphs() {
    ...
    //tira::tmesh<float> circle = tira::circle<float>(UI.glyph_tesselation).scale({ 0.0f, 0.0f });
    tira::tmesh circle = tira::circle(UI.glyph_tesselation).Scale({ 0.0f, 0.0f, 0.0f });


    const tira::tmesh glyph_row = circle.Tile({ 1.0f, 0.0f, 0.0f }, Tn.width());
    const tira::tmesh glyph_grid = glyph_row.Tile({ 0.0f, 1.0f, 0.0f }, Tn.height());
...
void InitCmapGeometry() {
    cmapGeometry = tira::glGeometry::GenerateRectangle();
}
