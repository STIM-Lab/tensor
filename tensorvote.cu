...
__global__ void kernelStickVote2D(glm::mat2* VT, glm::vec2* L, glm::vec2* V, glm::vec2 sigma, unsigned int power, float norm,
    int w, int s0, int s1) {
    ...
    glm::mat2 Votee = stickvote2(L, V, sigma, power, norm, w, s0, s1, glm::ivec2(x0, x1));
    VT[x0 * s1 + x1] = Votee;
}

__global__ void kernelPlateVote2D(glm::mat2* VT, glm::vec2* L, glm::vec2* V, glm::vec2 sigma, unsigned int power,
    int w, int s0, int s1) {
    ...
    glm::mat2 Votee = platevote2(L, V, sigma, power, w, s0, s1, glm::ivec2(x0, x1));
}
...
void cudaVote2D(float* input_field, float* output_field, unsigned int s0, unsigned int s1, float sigma, float sigma2, 
    ...
    if(STICK)
        ...
        kernelStickVote2D << < blocks, threads >> > ((glm::mat2*)gpuOutputField, (glm::vec2*)gpuL, (glm::vec2*)V, glm::vec2(sigma, sigma2), power, sn, w, s0, s1);
    //if (PLATE)
