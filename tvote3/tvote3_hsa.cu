...
glm::mat3* hsa_gaussian3(const glm::mat3* source, const unsigned int s0, const unsigned int s1, const unsigned int s2, const float sigma, glm::vec2 pixel_size,
    ...
    if (deviceID < 0) {
        ...
        float* gauss1 = tira::cpu::kernel_gaussian<float>(kernel_size0, 0, sigma, pixel_size[1]);
        ...
        float* gauss2 = tira::cpu::kernel_gaussian<float>(kernel_size0, 0, sigma, pixel_size[2]);
        glm::mat3* dest_2 = tira::cpu::convolve3<glm::mat3, float>(dest_1, out_s0, out_s1, out_s2, gauss2, 1, 1, kernel_size1, out_s0, out_s1, out_s2);
        free(gauss2);
        free(dest_1);
